...
vec3 Camera::World_Position(const ivec2& pixel_index)
{
    vec3 result;
    // TODO
    //tempC = Cell_Center(pixel_index);
     vec2 modC = Cell_Center(pixel_index);
     
     result = film_position + (horizontal_vector*modC[0]) + (vertical_vector*modC[1]);
     //film position + u * C_x + v * C_y
    
    //need to use the equation descriped in the lab to manipulate tempC 
    // so we have C_x and C_y :-)
